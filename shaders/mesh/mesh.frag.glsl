...
void main()
{
    ...
    vec3 last_col = vec3(view[3]); // get the last column
    vec3 camera_position = -O*last_col;

    // Compute the view direction
    vec3 view_direction = normalize(O[2]);
    ...
    // Circular Flashlight effect
    vec3 frag_to_camera = fragment.position - camera_position;
    float distance_to_center = length(frag_to_camera - dot(frag_to_camera, view_direction) * view_direction);
    
    float intensity = 1.0 - clamp(distance_to_center / flashlight_radius, 0.0, 1.0);
