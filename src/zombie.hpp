...
class zombie {
    public:
        ...
        zombie(vec3 _position, std::string _name, std::unordered_map<std::string, vec3>& obstacles);

        zombie(vec3 _position, std::string _name, bool _show, std::unordered_map<std::string, vec3>& obstacles);
        ...
        vec3 collide_with_object(vec3 object_pos, vec3 moving_direction, std::string obj_name);
